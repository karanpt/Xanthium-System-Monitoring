<html>
<title>All Time Average M4</title>
<head>
<meta charset="utf-8">
...
$sql = "SELECT AVG(SystemLoad) AS average FROM SystemLoad4";
...
$sql = "SELECT AVG(MemLoad) AS average FROM MemLoad4";
$result = $link->query($sql);
if($result->num_rows>0)
{
foreach($result as $row)
...
$sql = "SELECT AVG(NetLoad) AS average FROM NetLoad4";
$result = $link->query($sql);
if($result->num_rows>0)
{
foreach($result as $row)
